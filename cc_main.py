...
# KNN background subtraction method
# history: number of previous frames being compared
# threshold:
...
##########################  Thresholds  ########################################
    ...
    # Thresholding method_1
    mov = np.uint8(fgmask > 200) * 255

    # mov = np.uint8(fgmask)

    # trash, mov = cv2.threshold(mov, 100, 255, cv2.THRESH_BINARY)
...
#------------------------------------------------------------------------------#
    ...
    # Running total of cars "in" parking lot
    # total = abs(count_back - count_forward)
    ...
    # cv2.putText(frame_out, ("%d" % total), (620, 450),
    #                 cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2)
